...
// Based on reference implementation: borderlands4-serials/lib/byte_mirror
var (
	Uint2Mirror  [4]byte
	Uint3Mirror  [8]byte
	Uint4Mirror  [16]byte
...
// Mirror4Bits mirrors a 4-bit value using the precomputed lookup table
func Mirror4Bits(value byte) byte {
	return Uint4Mirror[value]
}

// Mirror8Bits mirrors an 8-bit value using the precomputed lookup table
func Mirror8Bits(value byte) byte {
	return Uint8Mirror[value]
}

// GenericMirror provides generic bit mirroring for any bit count
